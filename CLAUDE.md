...
You are implementing **Paper2Cloud**, a store paper-capture app that pushes Bills and Bill Payments into QuickBooks Desktop. Read `SPEC.md` before anything else, then `docs/FINDINGS-real-paper.md` — it describes what the store's paper actually looks like (checks photographed on top of invoices, handwritten adjusted totals, month-bracketed statements) and overrides any assumption that contradicts it. This file tells you how to work; the spec tells you what to build.
