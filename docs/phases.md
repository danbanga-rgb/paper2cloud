...
- [ ] Build the labeling helper: a tiny local page (`npm run dev` → `/label`) that shows an image beside the model's draft JSON and saves corrected JSON to `fixtures/labels/<image>.json` in the §6 shapes. (built 2026-09-24: editable fields not raw JSON, composite photos, multi-page sets, adjusted-total question, skip; drafts kept in `fixtures/drafts/`. Verified with the mock provider on synthetic images — not yet on the real 39.)
